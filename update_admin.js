...
type MenuType = 'qr' | 'courts' | 'usage' | 'settings' | 'clubs';
...
export default function AdminPage() {
    ...
    const fetchStadiums = async () => {
        ...
        const { data } = await supabase.from('stadiums').select('*').order('id', { ascending: true });
        if (data && data.length > 0) {
            setStadiums(data);
            setCurrentStadiumId(data[0].id);
        }
    };
                            ...
                            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-200">
                                ...
                                <form onSubmit={handleCreateAdmin} className="space-y-4 max-w-lg">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <label className="block text-xs font-bold text-slate-500 mb-1">비밀번호 (6자 이상)</label>
                                            <input
                                                type="password"
                                    ...
                                    <button 
                                        type="submit"
                                        className="py-2.5 px-6 bg-slate-800 hover:bg-slate-900 text-white font-bold rounded-lg text-xs transition-all shadow-md active:scale-[0.98]"
                                    >
                                        {newAdminEmail.trim() || newAdminPassword.trim() ? '💾 관리자 저장' : '➕ 관리자 추가'}
                                    </button>
                                </form>
                            </div>
