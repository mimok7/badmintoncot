...
### 전체 코드 정리 및 수정 필수 지침
- 모든 페이지·공통 코드·Supabase SQL 수정 시 [전체 페이지 코드 정리 체크리스트](./CODE_CLEANUP_CHECKLIST.md)를 반드시 확인합니다.
- 사용하지 않는 코드와 중복 코드를 먼저 확인하고, 기능 보존을 확인한 뒤 삭제·통합합니다.
- 수정 후 타입 검사, 빌드, 관련 페이지 기능 검증을 완료해야 합니다.
- 권한, RLS, 입장, 예약 관련 변경은 기능 검증과 보안 검토를 별도로 기록합니다.
- 운영 코드에 불필요한 `console.log`를 남기지 않습니다.

### Git 및 배포 브랜치 규칙
- 기본 작업·커밋·push·배포 브랜치는 `main`만 사용합니다.
- push 전 `git branch --show-current`가 `main`인지 확인합니다.
- `master` 브랜치는 사용하지 않습니다.
